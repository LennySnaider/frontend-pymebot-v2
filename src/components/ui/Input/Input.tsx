...
const Input = (props: InputProps) => {
    ...
    // Use useLayoutEffect to measure DOM elements once after render
    useLayoutEffect(() => {
        // Only proceed if there are prefix or suffix nodes
        if (!prefixNode.current && !suffixNode.current) {
            return
        }
        
        const prefixNodeWidth = prefixNode?.current?.offsetWidth || 0
        const suffixNodeWidth = suffixNode?.current?.offsetWidth || 0
        
        // Only update state if the measurements are different
    ...
    }, [prefix, suffix]) // Only re-run if prefix or suffix changes
